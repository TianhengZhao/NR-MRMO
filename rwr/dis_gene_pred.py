...
def edges_pre_TT(nodes_file, edge_train_test_file,
    ...
    for test_dis_name in test_dis_dic:
        ...
        if pre_node_id in ['1198', '1213']: continue
        if vec_type == 1:
...
def edges_pre_TT_main():
    pre = '../data/000/'
    edge_train_test_file = pre + 'input.txt'
    nodes_file = 'data/nodes_example.txt'
    #vectors_file = pre + 'emb.model'
    vectors_file = pre + 'emb.txt'
    pre_file = pre + 'n2v_embbeding_prediction_results.txt'
    vec_type = 0  # vec_type=1: vectors file from gensim; vec_type=0: from LINE.
    pre_times = 2
    top_n = 200
